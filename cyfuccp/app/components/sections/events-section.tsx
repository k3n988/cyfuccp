...
export function EventsSection() {
  return <section className="section events" id="events">
    ...
    <article className="featured-event" style={{ backgroundImage: `linear-gradient(90deg, rgba(20,25,55,.82), rgba(20,25,55,.16)), url("${imageUrls.worship}")` }}>
      <span className="event-tag">✦ FEATURED</span>
      <div><h3>Youth Worship Night</h3><p>◷ &nbsp;Aug 30, 2025 &nbsp; | &nbsp; ◷ &nbsp;6:00 PM<br />⌖ &nbsp; UCCP Bacolod</p><small>A night of praise, prayer and renewed purpose.<br />All are welcome!</small></div>
    </article>
    <div className="event-cards-row">
      {upcomingEvents.map(([title, image, date, time]) => <article className="event-card" key={title}>
        ...
        <div><h3>{title}</h3><p><CalendarDays size={13} /> {date}<br /><span>◷ &nbsp;{time}</span><br /><MapPin size={13} /> UCCP Bacolod</p></div>
      </article>)}
    </div>
    </div>
